...
import { reflect, type ReflectDeps } from './reflector.js';
import { exportAll } from './export.js';

export interface BackfillOptions {
  project?: string;
  minMessages?: number;   // skip trivial sessions (default 8)
  limit?: number;         // cap the batch (default 50)
  since?: string;         // ISO date — only sessions active on/after this
  force?: boolean;        // re-analyze sessions already reflected
  dryRun?: boolean;       // report the selection, run nothing
}
...
export function selectBackfillSessions(db: Database.Database, opts: BackfillOptions): SessionRow[] {
  const minMessages = opts.minMessages ?? 8;
  const limit = opts.limit ?? 50;

  const where: string[] = ['jsonl_path IS NOT NULL', 'message_count >= @minMessages'];
  const params: Record<string, unknown> = { minMessages, limit };
  if (!opts.force) where.push('last_reflected_index = 0');     // only never-analyzed sessions
  if (opts.project) { where.push('project = @project'); params.project = opts.project; }
  if (opts.since) { where.push('(last_active >= @since OR started_at >= @since)'); params.since = opts.since; }
...
export async function backfillSessions(
  ...
  for (const s of sessions) {
    ...
    try {
      ...
      if (r.skipped) result.skippedNoSignal++;
    } catch (err) {
      console.warn(`[backfill] session ${s.session_id} failed:`, (err as Error).message);
    }
  }

  if (result.inserted > 0) exportAll(db);
  return result;
}
