 ...
 */
function getSourceType(filePath: string): SourceType | null {
  const config = getClaudeConfig();
  const normalized = filePath.replace(/\\/g, '/');

  if (normalized.includes('/agents/')) return 'agent_def';
  if (normalized.includes('/skills/') && basename(filePath) === 'SKILL.md') return 'skill_def';
  if (normalized.includes('/plans/')) return 'plan_file';
  if (normalized.includes('/memory/')) return 'memory_file';
  return null;
}
...
export function startWatcher(options?: {
  ...
  const handleChange = (filePath: string, event: string) => {
    ...
    try {
      ...
      console.error(`[${event}] Failed to reindex ${filePath}:`, (err as Error).message);
    }
  };
