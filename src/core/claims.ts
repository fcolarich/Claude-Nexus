...
export function computeClaimId(claim_type: string, fact: string): string {
...
export function insertClaim(db: Database.Database, input: ClaimInput): { id: string; inserted: boolean } {
...
export function listClaimsForMemory(
	...
	return (db.prepare(sql).all(memory_id) as Record<string, unknown>[]).map(rowToClaim);
}
 ...
 */
export function markClaimInvalid(db: Database.Database, id: string, supersededByClaimId?: string): boolean {
	const res = db.prepare(
		`UPDATE claims SET valid_until = datetime('now'), expired_at = datetime('now') WHERE id = ? AND valid_until IS NULL`
	).run(id);
	if (res.changes === 0) return false;

	if (supersededByClaimId) {
		db.prepare(
			`INSERT OR IGNORE INTO memory_links (source_id, target_id, link_type, confidence) VALUES (?, ?, 'supersedes', 1.0)`
