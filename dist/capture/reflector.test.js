...
const asstMsg = (content) => ({ type: 'assistant', message: { role: 'assistant', content } });
...
const countMemories = (db) => db.prepare(`SELECT COUNT(*) AS c FROM memories`).get().c;
describe('reflect', () => {
    it('extracts and inserts distinct memories, advancing the cursor', async () => {
        const db = freshDb();
        const p = makeTranscript(SIGNAL_TRANSCRIPT);
        const r = await reflect(db, { session_id: 's1', transcript_path: p, project: 'proj' }, { extract: async () => [candA, candB], embed: async (t) => vecFromText(t) });
        expect(r.skipped).toBe(false);
        expect(r.inserted).toBe(2);
        expect(r.merged).toBe(0);
        expect(countMemories(db)).toBe(2);
        const cursor = db.prepare(`SELECT last_reflected_index FROM sessions WHERE session_id='s1'`).get().last_reflected_index;
        expect(cursor).toBe(SIGNAL_TRANSCRIPT.length);
        const approved = db.prepare(`SELECT COUNT(*) AS c FROM memories WHERE review_status='approved'`).get().c;
    ...
    it('returns touchedIds naming every inserted/merged memory, for scoped export', async () => {
        const db = freshDb();
        const p = makeTranscript(SIGNAL_TRANSCRIPT);
        const r = await reflect(db, { session_id: 's1b', transcript_path: p, project: 'proj' }, { extract: async () => [candA, candB], embed: async (t) => vecFromText(t) });
        const ids = db.prepare(`SELECT id FROM memories`).all().map((row) => row.id).sort();
    ...
    it('skips a trivial window via the Observer gate', async () => {
        ...
        const r = await reflect(db, { session_id: 's2', transcript_path: p, project: 'proj' }, { extract: async () => [candA], embed: async (t) => vecFromText(t) });
    ...
    it('control — the same transcript without the marker DOES reach the extractor', async () => {
        ...
        let called = false;
        const r = await reflect(db, { session_id: 'sched-control', transcript_path: p, project: 'proj' }, { extract: async () => { called = true; return []; }, embed: async () => null });
    ...
    it('never extracts from a denylisted scheduled-task session', async () => {
        ...
        let called = false;
        const r = await reflect(db, { session_id: 'sched-1', transcript_path: p, project: 'proj' }, { extract: async () => { called = true; return []; }, embed: async () => null });
    ...
    it('does not re-process transcript lines on a second run', async () => {
        const db = freshDb();
        const p = makeTranscript(SIGNAL_TRANSCRIPT);
        const deps = { extract: async () => [candA], embed: async (t) => vecFromText(t) };
    ...
    it('merges an exact-duplicate candidate instead of re-inserting', async () => {
        const db = freshDb();
        const p = makeTranscript(SIGNAL_TRANSCRIPT);
        const r = await reflect(db, { session_id: 's4', transcript_path: p, project: 'proj' }, { extract: async () => [candA, candA], embed: async (t) => vecFromText(t) });
    ...
    it('merges a semantically near-duplicate candidate', async () => {
        ...
        const r = await reflect(db, { session_id: 's5', transcript_path: p, project: 'proj' }, { extract: async () => [candA, candC], embed: async () => FIXED_VEC });
        expect(r.inserted).toBe(1);
        expect(r.merged).toBe(1);
        expect(countMemories(db)).toBe(1);
        db.close();
    ...
    it('marks low-confidence memories pending review', async () => {
        const db = freshDb();
        const p = makeTranscript(SIGNAL_TRANSCRIPT);
        await reflect(db, { session_id: 's6', transcript_path: p, project: 'proj' }, { extract: async () => [{ ...candA, confidence: 0.5 }], embed: async (t) => vecFromText(t) });
        const status = db.prepare(`SELECT review_status FROM memories LIMIT 1`).get().review_status;
    ...
    describe('Gate 1 — pre-extraction secret redaction (D-005)', () => {
        ...
        function assertGate1Scrubbed(receivedText) {
        ...
        it('scrubs strict-kind secrets from extractor input when vcc succeeds', async () => {
            ...
            let receivedText = '';
            const fakeVcc = {
                compactWindowLines: () => ({ ok: true, text: SECRET_WINDOW_TEXT }),
                compactFileInPlace: () => ({ ok: true, text: 'shrunk' }),
            };
    ...
    describe('Gate 2 — post-extraction secret redaction (D-010, D-012)', () => {
        ...
        const INSIGHT_SENTENCE = 'Rotate compromised keys immediately and audit IAM permissions after every incident.';
        ...
        it('redacts title/body/tags before insertMemory, preserves insight prose, drops offending tags, and embeds redacted body', async () => {
            ...
            expect(r.inserted).toBe(1);
            const row = db.prepare(`SELECT title, body, tags FROM memories LIMIT 1`).get();
    ...
    describe('ReflectResult observability — redactions / redaction_kinds (D-007, D-008)', () => {
        ...
        it('reports the raw span count and sorted unique kind list across both gates on the full path', async () => {
            ...
            const r = await reflect(db, { session_id: 'obs-full', transcript_path: p, project: 'proj' }, { extract: async () => [candWithSecretTag], embed: async (t) => vecFromText(t) });
        ...
        it('reports zero redactions and an empty kind list on a clean window', async () => {
            const db = freshDb();
            const p = makeTranscript(SIGNAL_TRANSCRIPT);
            const r = await reflect(db, { session_id: 'obs-clean', transcript_path: p, project: 'proj' }, { extract: async () => [candA], embed: async (t) => vecFromText(t) });
        ...
        it('leaves redactions/redaction_kinds undefined on the origin-gate early return', async () => {
            const db = freshDb();
            const p = makeTranscript(schedTranscript(SCHED_MARKER));
            const r = await reflect(db, { session_id: 'obs-origin-gate', transcript_path: p, project: 'proj' }, { extract: async () => [], embed: async () => null });
            expect(r.skipped).toBe(true);
        ...
        it('leaves redactions/redaction_kinds undefined on the Observer-gate early return', async () => {
            const db = freshDb();
            const p = makeTranscript([userMsg('hi')]);
            const r = await reflect(db, { session_id: 'obs-observer-gate', transcript_path: p, project: 'proj' }, { extract: async () => [candA], embed: async (t) => vecFromText(t) });
            expect(r.skipped).toBe(true);
            expect(r.redactions).toBeUndefined();
            expect(r.redaction_kinds).toBeUndefined();
            db.close();
        ...
        describe('summary log line', () => {
            it('logs exactly one redaction summary line, with no secret value in the log arguments', async () => {
                ...
                try {
                    await reflect(db, { session_id: 'obs-log-hit', transcript_path: p, project: 'proj' }, { extract: async () => [candWithSecretTag], embed: async (t) => vecFromText(t) });
                    const redactionLines = logSpy.mock.calls.filter((call) => typeof call[0] === 'string' && call[0].includes('redacted') && call[0].includes('secret span'));
            ...
            it('logs no redaction summary line when redactions === 0', async () => {
                ...
                const logSpy = vi.spyOn(console, 'log').mockImplementation(() => { });
                try {
                    await reflect(db, { session_id: 'obs-log-miss', transcript_path: p, project: 'proj' }, { extract: async () => [candA], embed: async (t) => vecFromText(t) });
                    const redactionLines = logSpy.mock.calls.filter((call) => typeof call[0] === 'string' && call[0].includes('redacted') && call[0].includes('secret span'));
    ...
    describe('Fail-open — deps.redact throwing (D-002, D-009)', () => {
        ...
        it('resolves without rejecting when deps.redact throws unconditionally, leaving extractor input and inserted candidate text unmodified', async () => {
            ...
            let receivedText = '';
            const alwaysThrows = (_text, _mode) => {
        ...
        it('contains gate 1 and gate 2 fail-open independently when deps.redact throws only on its second call (gate 1 succeeds, gate 2 fails)', async () => {
            ...
            const throwsFromSecondCall = (text, _mode) => {
                callCount++;
                if (callCount >= 2) {
    ...
    describe('Fix 1 — ADR-reference demotion (supersede-insert)', () => {
        ...
        it('happy path — upgrades a decision to a reference when a real ADR id is later cited', async () => {
            ...
            const r1 = await reflect(db, { session_id: 'u1', transcript_path: p1, project: 'proj', cwd }, { extract: async () => [decisionCand], embed: async () => FIXED_VEC });
            expect(r1.inserted).toBe(1);
            const decisionId = db.prepare(`SELECT id FROM memories WHERE memory_type = 'decision'`).get().id;
            const p2 = makeTranscript(SIGNAL_TRANSCRIPT);
            const r2 = await reflect(db, { session_id: 'u2', transcript_path: p2, project: 'proj', cwd }, { extract: async () => [refCand], embed: async () => FIXED_VEC });
            expect(r2.upgraded).toBe(1);
            expect(r2.merged).toBe(0);
            const decisionRow = db.prepare(`SELECT superseded_by FROM memories WHERE id = ?`).get(decisionId);
            expect(decisionRow.superseded_by).not.toBeNull();
            const newRow = db.prepare(`SELECT id FROM memories WHERE memory_type = 'reference'`).get();
            expect(newRow?.id).toBe(decisionRow.superseded_by);
            const queueCount = db.prepare(`SELECT COUNT(*) AS c FROM memories WHERE promotion_target != 'none' AND promoted_to IS NULL AND superseded_by IS NULL`).get().c;
        ...
        it('negative — reference candidate has no ADR/DDR id in the body: touch-only', async () => {
            const db = freshDb();
            const p1 = makeTranscript(SIGNAL_TRANSCRIPT);
            await reflect(db, { session_id: 'v1', transcript_path: p1, project: 'proj' }, { extract: async () => [decisionCand], embed: async () => FIXED_VEC });
            const decisionId = db.prepare(`SELECT id FROM memories WHERE memory_type = 'decision'`).get().id;
            const p2 = makeTranscript(SIGNAL_TRANSCRIPT);
            const r2 = await reflect(db, { session_id: 'v2', transcript_path: p2, project: 'proj' }, { extract: async () => [noRefCand], embed: async () => FIXED_VEC });
        ...
        it('negative — matched row is not a decision (e.g. convention): touch-only', async () => {
            const db = freshDb();
            const conventionCand = { ...candA }; // memory_type: 'convention'
            const p1 = makeTranscript(SIGNAL_TRANSCRIPT);
            await reflect(db, { session_id: 'w1', transcript_path: p1, project: 'proj' }, { extract: async () => [conventionCand], embed: async () => FIXED_VEC });
            const conventionId = db.prepare(`SELECT id FROM memories WHERE memory_type = 'convention'`).get().id;
            const p2 = makeTranscript(SIGNAL_TRANSCRIPT);
            const r2 = await reflect(db, { session_id: 'w2', transcript_path: p2, project: 'proj' }, { extract: async () => [refCand], embed: async () => FIXED_VEC });
            expect(r2.merged).toBe(1);
            expect(r2.upgraded).toBe(0);
            const conventionRow = db.prepare(`SELECT superseded_by FROM memories WHERE id = ?`).get(conventionId);
        ...
        it('positive — broadened trigger: promotion_target=\'none\' decision row also gets superseded', async () => {
            const db = freshDb();
            const cwd = makeDecisions(['042']);
            const noneDecisionCand = { ...decisionCand, promotion_target: 'none' };
            const p1 = makeTranscript(SIGNAL_TRANSCRIPT);
            await reflect(db, { session_id: 'x1', transcript_path: p1, project: 'proj', cwd }, { extract: async () => [noneDecisionCand], embed: async () => FIXED_VEC });
            const decisionId = db.prepare(`SELECT id FROM memories WHERE memory_type = 'decision'`).get().id;
            const p2 = makeTranscript(SIGNAL_TRANSCRIPT);
            const r2 = await reflect(db, { session_id: 'x2', transcript_path: p2, project: 'proj', cwd }, { extract: async () => [refCand], embed: async () => FIXED_VEC });
            expect(r2.upgraded).toBe(1);
            expect(r2.merged).toBe(0);
            const decisionRow = db.prepare(`SELECT superseded_by FROM memories WHERE id = ?`).get(decisionId);
            expect(decisionRow.superseded_by).not.toBeNull();
        ...
        it('idempotency / convergence — a third window matching the reference row is touch-only, no re-supersede', async () => {
            ...
            const cwd = makeDecisions(['042']);
            const p1 = makeTranscript(SIGNAL_TRANSCRIPT);
            await reflect(db, { session_id: 'y1', transcript_path: p1, project: 'proj', cwd }, { extract: async () => [decisionCand], embed: async () => FIXED_VEC });
            const p2 = makeTranscript(SIGNAL_TRANSCRIPT);
            const r2 = await reflect(db, { session_id: 'y2', transcript_path: p2, project: 'proj', cwd }, { extract: async () => [refCand], embed: async () => FIXED_VEC });
            expect(r2.upgraded).toBe(1);
            expect(countMemories(db)).toBe(2); // decision (superseded) + reference
            const referenceId = db.prepare(`SELECT id FROM memories WHERE memory_type = 'reference'`).get().id;
            const p3 = makeTranscript(SIGNAL_TRANSCRIPT);
            const r3 = await reflect(db, { session_id: 'y3', transcript_path: p3, project: 'proj', cwd }, { extract: async () => [refCand], embed: async () => FIXED_VEC });
            ...
            const referenceRow = db.prepare(`SELECT superseded_by FROM memories WHERE id = ?`).get(referenceId);
        ...
        it('rollback — a throw during the supersede UPDATE persists neither the new row nor superseded_by', async () => {
            ...
            const cwd = makeDecisions(['042']);
            const p1 = makeTranscript(SIGNAL_TRANSCRIPT);
            await reflect(db, { session_id: 'z1', transcript_path: p1, project: 'proj', cwd }, { extract: async () => [decisionCand], embed: async () => FIXED_VEC });
            const decisionId = db.prepare(`SELECT id FROM memories WHERE memory_type = 'decision'`).get().id;
            ...
            db.prepare = spy;
            const p2 = makeTranscript(SIGNAL_TRANSCRIPT);
            await expect(reflect(db, { session_id: 'z2', transcript_path: p2, project: 'proj', cwd }, { extract: async () => [refCand], embed: async () => FIXED_VEC })).rejects.toThrow();
        ...
        it('negative — hallucinated citation: candidate cites a plausible but nonexistent ADR id: touch-only', async () => {
            ...
            const hallucinatedRefCand = {
                ...refCand, body: 'Supersede-insert dedup upgrade decision → ADR-999',
            };
            const p1 = makeTranscript(SIGNAL_TRANSCRIPT);
            await reflect(db, { session_id: 'aa1', transcript_path: p1, project: 'proj', cwd }, { extract: async () => [decisionCand], embed: async () => FIXED_VEC });
            const decisionId = db.prepare(`SELECT id FROM memories WHERE memory_type = 'decision'`).get().id;
            const p2 = makeTranscript(SIGNAL_TRANSCRIPT);
            const r2 = await reflect(db, { session_id: 'aa2', transcript_path: p2, project: 'proj', cwd }, { extract: async () => [hallucinatedRefCand], embed: async () => FIXED_VEC });
            expect(r2.upgraded).toBe(0);
            expect(r2.merged).toBe(1);
            const decisionRow = db.prepare(`SELECT superseded_by FROM memories WHERE id = ?`).get(decisionId);
            expect(decisionRow.superseded_by).toBeNull();
        ...
        it('negative — no doc-spine (opts.cwd omitted): fail-closed, touch-only despite a real-shaped citation', async () => {
            const db = freshDb();
            const p1 = makeTranscript(SIGNAL_TRANSCRIPT);
            await reflect(db, { session_id: 'bb1', transcript_path: p1, project: 'proj' }, { extract: async () => [decisionCand], embed: async () => FIXED_VEC });
            const decisionId = db.prepare(`SELECT id FROM memories WHERE memory_type = 'decision'`).get().id;
            const p2 = makeTranscript(SIGNAL_TRANSCRIPT);
