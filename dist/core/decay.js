...
function ageDays(lastVerifiedAt) {
...
export function decayFactor(decayClass, lastVerifiedAt) {
...
export function effectiveConfidence(m) {
 ...
 */
export function flagStaleMemories(db, threshold) {
    const memoriesExists = db.prepare(`SELECT name FROM sqlite_master WHERE type='table' AND name='memories'`).get();
    if (!memoriesExists)
        return 0;
  ...
  `).all();
    ...
    const ins = db.prepare(`INSERT INTO diagnostics (type, atom_id, message, details) VALUES ('stale', NULL, ?, ?)`);
    for (const m of stale) {
        ins.run(`Memory decayed below recall threshold: ${m.title}`, JSON.stringify({ memory_id: m.id, effective_confidence: Number(effectiveConfidence(m).toFixed(3)) }));
    }
    return stale.length;
}
