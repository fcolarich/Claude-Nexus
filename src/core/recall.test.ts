...
vi.mock('gpt-tokenizer', async () => {
  const actual = await vi.importActual<typeof import('gpt-tokenizer')>('gpt-tokenizer');
  return { ...actual, encode: vi.fn((s: string) => actual.encode(s)) };
});
...
describe('recallByQuery RRF fusion (SC-5)', () => {
  ...
  it('reranker receives the fused set including FTS5-only memories', async () => {
    ...
    const spyRerank = vi.fn(async (_q: string, documents: string[]) => {
