...
import { buildBm25Corpus, rrfMerge, upsertLink, linkAtom, } from './links.js';
...
function insertAtom(db, id, title, body, opts = {}) {
    db.prepare(`
  ...
  `).run(id, title, body, id, id + body, opts.linkedAt ?? null);
    if (opts.withVec) {
        ...
        const row = db.prepare(`SELECT rowid FROM atoms WHERE id = ?`).get(id);
        try {
...
describe('rrfMerge', () => {
    ...
    it('returns no more than topK items', () => {
        const bm25 = Array.from({ length: 20 }, (_, i) => ({ id: `b${i}`, score: 1 - i * 0.05 }));
        const dense = Array.from({ length: 20 }, (_, i) => ({ id: `d${i}`, score: 1 - i * 0.05 }));
...
describe('upsertLink', () => {
    it('inserts two rows (A→B and B→A) per call', () => {
        ...
        const rows = db.prepare(`SELECT source_id, target_id FROM atom_links WHERE link_type='related'`).all();
    ...
    it('second identical call leaves row count unchanged (idempotent)', () => {
        ...
        upsertLink(db, 'a', 'b', 'related', 0.8);
        const before = db.prepare(`SELECT COUNT(*) AS c FROM atom_links`).get().c;
        upsertLink(db, 'a', 'b', 'related', 0.8);
        const after = db.prepare(`SELECT COUNT(*) AS c FROM atom_links`).get().c;
    ...
    it('self-link (A→A) inserts zero rows', () => {
        ...
        const count = db.prepare(`SELECT COUNT(*) AS c FROM atom_links`).get().c;
        expect(count).toBe(0);
        db.close();
    });
});
...
describe('linkAtom', () => {
    ...
    it('never writes a self-link even when corpus contains the same atom', async () => {
        ...
        const selfLinks = db.prepare(`SELECT * FROM atom_links WHERE source_id = 'target' AND target_id = 'target'`).all();
    ...
    it('updates atoms.linked_at after processing', async () => {
        const db = freshDb();
        for (let i = 0; i < 5; i++)
            insertAtom(db, `s${i}`, `S ${i}`, `body ${i}`);
        insertAtom(db, 'x', 'X', 'body x');
        await linkAtom(db, 'x', embedFn);
        const row = db.prepare(`SELECT linked_at FROM atoms WHERE id = 'x'`).get();
