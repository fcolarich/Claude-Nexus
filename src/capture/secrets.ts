...
export const SECRET_PATTERNS: ReadonlyArray<SecretPatternRow> = [
	{
...
const HIGH_ENTROPY_CONTEXT_CUE_RE =
...
function hasTwoCharClasses(token: string): boolean {
	let classes = 0;
 ...
 */
function applyHighEntropyBackstop(text: string, redactions: string[]): string {
	let result = '';
	let lastIndex = 0;
	...
	for (const match of Array.from(text.matchAll(HIGH_ENTROPY_CANDIDATE_RE))) {
		...
		if (!hasTwoCharClasses(token)) continue;
		if (HEX_ONLY_RE.test(token)) continue;
		if (UUID_SHAPE_RE.test(token)) continue;
		...
		if (!HIGH_ENTROPY_CONTEXT_CUE_RE.test(context)) continue;

		if (shannonEntropy(token) < HIGH_ENTROPY_MIN_BITS) continue;
 ...
 */
export function redactCandidate<T extends MemoryCandidateLike>(
	c: T,
	redact: typeof redactSecrets = redactSecrets,
): { candidate: T; redactions: string[] } {
 ...
 */
function replaceGroupSpan(text: string, row: SecretPatternRow, redactions: string[]): string {
	const groupIndex = row.group as number;
	...
	for (const match of Array.from(text.matchAll(re))) {
		...
		if (row.suppress?.(value)) continue;
 ...
 */
export function redactSecrets(text: string, mode: RedactionMode = 'full'): RedactionResult {
