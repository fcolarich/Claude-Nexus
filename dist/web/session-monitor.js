 ...
 */
function getRunningClaudePids() {
    const sessionIds = new Set();
    try {
        // On Windows, look for node processes running Claude Code
        const output = execSync('wmic process where "name=\'node.exe\'" get CommandLine /format:list', { encoding: 'utf-8', timeout: 5000 });
 ...
 */
function parseSessionTail(jsonlPath) {
    ...
    try {
        ...
        if (lastTimestamp) {
            ...
            if (age < FIVE_MINUTES) {
                if (hasQueueEnqueue)
                    return { status: 'waiting_input', pendingPrompt, lastTimestamp };
                if (lastEntryType === 'assistant')
                    return { status: 'waiting_input', pendingPrompt: null, lastTimestamp };
 ...
 */
export function refreshSessionStatuses(db) {
  ...
  `);
    const runAll = db.transaction(() => {
        for (const session of sessions) {
            ...
            update.run(state.status, state.pendingPrompt, state.lastTimestamp, session.session_id);
        }
    });
    runAll();
}
