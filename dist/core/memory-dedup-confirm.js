 ...
 */
function identifiersConflict(factA, factB) {
...
async function ensureClaims(db, memory, callFn) {
    const row = db.prepare(`SELECT claims_extracted_at FROM memories WHERE id = ?`).get(memory.id);
    if (row?.claims_extracted_at)
        return; // already decomposed — reuse persisted claims
    const source = { id: memory.id, body: memory.body, memory_type: memory.memory_type, confidence: memory.confidence };
...
async function ensureEmbedded(db, claimId, embedFn) {
    const rowid = db.prepare(`SELECT rowid FROM claims WHERE id = ?`).get(claimId).rowid;
    const hasVec = db.prepare(`SELECT 1 FROM claims_vec WHERE rowid = ?`).get(rowid);
    if (!hasVec)
        await embedClaim(db, claimId, embedFn);
}
/**
 ...
 */
export async function confirmMemoryDuplicate(db, memoryA, memoryB, callFn, embedFn = generateEmbedding) {
    ...
    for (const a of claimsA) {
        for (const b of claimsB) {
            if (detectNumericContradiction(a.fact, b.fact))
                return 'contradicts';
        }
    }
    for (const c of [...claimsA, ...claimsB])
        await ensureEmbedded(db, c.id, embedFn);
    let matched = 0;
    for (const a of claimsA) {
        const found = claimsB.some((b) => {
            if (identifiersConflict(a.fact, b.fact))
                return false;
...
//# sourceMappingURL=memory-dedup-confirm.js.map