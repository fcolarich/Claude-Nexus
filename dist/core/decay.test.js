...
describe('flagStaleMemories', () => {
    ...
    it('flags decayed memories and rewrites stale diagnostics idempotently', () => {
        ...
        const diags = db.prepare(`SELECT details FROM diagnostics WHERE type='stale'`).all();
        ...
        const count = db.prepare(`SELECT COUNT(*) c FROM diagnostics WHERE type='stale'`).get().c;
        expect(count).toBe(1);
        db.close();
    });
});
