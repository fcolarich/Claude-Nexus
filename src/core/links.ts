...
export function buildBm25Corpus(
  ...
  index.definePrepTasks([(t: string) => t.toLowerCase().split(/\W+/).filter(Boolean)]);
...
export function upsertLink(
  ...
  linkType: LinkType,
  confidence: number,
  table: 'atom_links' | 'memory_links' = 'atom_links'
): void {
  if (sourceId === targetId) return;

  const stmt = db.prepare(
    `INSERT OR IGNORE INTO ${table} (source_id, target_id, link_type, confidence)
     VALUES (?, ?, ?, ?)`
  );
    ...
    if (vecTable) {
      ...
      const knnRows = db.prepare(
        `SELECT a.id, av.distance FROM atoms_vec av
         ...
         LIMIT ${KNN_LIMIT}`
      ).all(blob, atomId) as { id: string; distance: number }[];
  ...
  if (effectiveCorpus) {
    try {
      ...
      for (const [ref, score] of bm25Raw) {
        if (ref !== atomId) bm25Results.push({ id: ref, score });
      }
    } catch {
...
export async function linkMemory(
  ...
  try {
    ...
    if (vecTable) {
      const blob = vecToBlob(normalize(vec));
      const knnRows = db.prepare(
        `SELECT m.id, mv.distance FROM memories_vec mv
         ...
         LIMIT ${KNN_LIMIT}`
      ).all(blob, memoryId) as { id: string; distance: number }[];
  ...
  if (effectiveCorpus) {
    try {
      ...
      const bm25Raw = effectiveCorpus.search(query, KNN_LIMIT);
      for (const [ref, score] of bm25Raw) {
        if (ref !== memoryId) bm25Results.push({ id: ref, score });
      }
    } catch {
      // BM25 search failed
    }
...
function buildFallbackAtomCorpus(db: Database.Database): Bm25Index | null {
  try {
    const atoms = db.prepare(
      `SELECT id, title, body FROM atoms LIMIT 1000`
