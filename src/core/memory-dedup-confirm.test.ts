...
function insertConfirmable(db: ReturnType<typeof freshDb>, title: string, body: string) {
...
function fakeEmbedFn(overrides: Record<string, Float32Array> = {}) {
...
describe('confirmMemoryDuplicate', () => {
	it('returns "contradicts" when any claim pair carries a numeric contradiction, without checking similarity', async () => {
		...
		const callFn = async (_s: string, user: string) => {
	...
	it('returns "confirmed" when a majority of A\'s claims match a claim in B (same wording, same vector)', async () => {
		...
		const callFn = async (_s: string, user: string) => JSON.stringify([{ fact: user.includes('restated') ? factB : factA }]);
		...
		const sharedVec = new Float32Array(1024); sharedVec[0] = 1;
	...
	it('returns "confirmed" for independently-paraphrased duplicate claims scoring below claim-dedup\'s own flag threshold (0.92)', async () => {
		...
		const callFn = async (_s: string, user: string) => JSON.stringify([{ fact: user.includes('mechanism') ? factA : factB }]);
		...
		const vecA = new Float32Array(1024); vecA[0] = 0.884; vecA[1] = Math.sqrt(1 - 0.884 ** 2);
