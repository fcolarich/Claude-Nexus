 ...
 */
export function buildProjectAliases(db) {
...
export function applyProjectAliases(db, aliases, dryRun) {
    const report = {
        aliases, memoriesUpdated: 0, atomsUpdated: 0, sessionsUpdated: 0, merged: 0, dryRun,
    };
    if (dryRun || aliases.length === 0)
        return report;
    const recordAlias = db.prepare(`INSERT OR REPLACE INTO project_aliases (alias_slug, canonical_slug) VALUES (?, ?)`);
...
export async function migrateProjects(db, opts, deps) {
...
//# sourceMappingURL=project-migrate.js.map