...
describe('identifiersDisjoint', () => {
	...
	it('is true when both facts carry identifiers and the sets share none', () => {
		expect(identifiersDisjoint(
			'The doc-sync system regenerates notes.md.',
			'The doc-sync system regenerates design.md.',
		)).toBe(true);
	});

	it('is true for two different function names embedded in otherwise-identical sentences', () => {
		expect(identifiersDisjoint(
			'The GetTreeBaseGustWind function needs guarding against numeric overflow using max(period, 1e-5f).',
			'The GetBaseGustWind function needs guarding against numeric overflow using max(period, 1e-5f).',
		)).toBe(true);
	});
	...
	it('is true when the facts share some identifiers but differ on the one that matters', () => {
		...
		// these the same fact.
		expect(identifiersDisjoint(
			'The `ImpactCollisionSystem`, `PaintCommandBufferSystem`, and `AlphaTextureUploadSystem` synchronize via `ApplyStampJob.Run()`.',
			'The `ImpactCollisionSystem`, `PaintCommandBufferSystem`, and `AlphaTextureUploadSystem` synchronize via `ApplyChanges()`.',
		)).toBe(true);
	});

	it('is false when the facts have exactly the same identifier set', () => {
		expect(identifiersDisjoint(
			'src/core/distill.ts computes identifiers.',
			'src/core/distill.ts also computes identifiers before insertMemory.',
		)).toBe(false);
	});

	it('is false when neither fact has any identifiers (nothing to veto on)', () => {
		expect(identifiersDisjoint(
			'Substantive coverage means multiple sentences or a dedicated section.',
			'Substantive coverage means multiple sentences or a dedicated section within the source.',
		)).toBe(false);
	});

	it('is false when only one fact carries identifiers', () => {
		expect(identifiersDisjoint(
			'src/core/distill.ts computes mergedIdentifiers.',
			'Substantive coverage means multiple sentences or a dedicated section.',
		)).toBe(false);
	});
});
