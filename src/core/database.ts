...
function migratePromotionClassification(db: Database.Database): void {
  try { db.exec(`ALTER TABLE memories ADD COLUMN promotion_target TEXT NOT NULL DEFAULT 'none' CHECK(promotion_target IN ('none','adr','ddr','best_practice','recipe','note'))`); } catch {}
  try { db.exec(`ALTER TABLE memories ADD COLUMN promoted_to TEXT`); } catch {}
...
function migrateCoworkSupport(db: Database.Database): void {
  try { db.exec(`ALTER TABLE sessions ADD COLUMN is_cowork INTEGER DEFAULT 0`); } catch {}
  try { db.exec(`ALTER TABLE sessions ADD COLUMN workspace_id TEXT`); } catch {}
...
function migrateClaimsTable(db: Database.Database): void {
  ...
  if (!schemaRow) return;
  if (schemaRow.sql.includes("'same_as'")) return; // already migrated
...
function migrateClaimsVec(db: Database.Database): void {
  try {
    ...
    console.warn('[claude-nexus] Could not create claims_vec table — claim dedup embeddings disabled:', (err as Error).message);
  }
}
