...
function seedLink(db, sourceId, targetId, linkType) {
    db.prepare(`
...
function countDiagnostics(db, type) {
    const row = db.prepare(`SELECT COUNT(*) AS c FROM diagnostics WHERE type = ?`).get(type);
    return row.c;
}
// ── Fake embedFn ──────────────────────────────────────────────────────
...
async function fakeEmbedFn(text) {
    if (text.includes(DUP_MARKER))
        return seededVector('fixed-dup-vector');
    return seededVector(text);
}
// ── Fake haikuFn ──────────────────────────────────────────────────────
function makeConflictFake(reason = 'divergent facts') {
...
describe('consolidateMemories', () => {
    it('runs all five phases and returns aggregated counts matching the seeded scenario', async () => {
        ...
        const contradicts = db.prepare(`SELECT COUNT(*) AS c FROM memory_links WHERE link_type = 'contradicts'`).get();
    ...
    it('has no schema drift in the tables consolidateMemories touches', () => {
        ...
        const memoriesCols = db.prepare(`PRAGMA table_info('memories')`).all()
            .map((r) => r.name).sort();
        ...
        function checkValues(sql, column) {
            ...
            if (!m)
                throw new Error(`no CHECK(${column} IN (...)) clause found`);
...
//# sourceMappingURL=consolidate.test.js.map