...
import { searchSession, getStats } from '../core/search.js';
...
function runSearchSessionHandler(db, sessionId, query, maxMatches) {
...
describe('nexus_stats session-search counts line', () => {
    function freshStatsDb() {
        const db = openDatabase(':memory:');
        initializeSchema(db);
        return db;
    }
    function insertLogRow(db, source, n = 1) {
        for (let i = 0; i < n; i++) {
