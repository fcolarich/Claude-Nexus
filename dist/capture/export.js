...
function memoryFileName(m) {
    return `${m.id}-${slug(m.title)}.md`;
}
function memoryFileContent(m) {
...
function buildIndexContent(bucket, mems, cap) {
    ...
    const index = [`# Memory — ${bucket}`, '', `_${totalCount} memories · generated by Claude Nexus_`, ''];
...
function approvedMemoriesInBucket(db, bucket) {
 ...
 */
function exportTouched(db, exportDir, cap, touchedIds) {
    const placeholders = touchedIds.map(() => '?').join(',');
    const touchedRows = db.prepare(`SELECT * FROM memories WHERE id IN (${placeholders})`).all(...touchedIds);
 ...
 */
export function exportAll(db, exportDirOverride, options) {
