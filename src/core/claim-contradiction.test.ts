...
describe('writeContradictionLinks', () => {
	...
	it('is surfacing-only — never deletes, hides, or supersedes either claim', () => {
		...
		const a = db.prepare(`SELECT valid_until, fact FROM claims WHERE id = 'claim-a'`).get() as { valid_until: string | null; fact: string };
		const b = db.prepare(`SELECT valid_until, fact FROM claims WHERE id = 'claim-b'`).get() as { valid_until: string | null; fact: string };
	...
	it('is idempotent — calling it twice for the same pair does not throw or duplicate rows', () => {
		...
		const count = (db.prepare(`SELECT COUNT(*) c FROM memory_links WHERE link_type = 'contradicts'`).get() as { c: number }).c;
