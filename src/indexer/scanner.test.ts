...
function seedSession(db: DatabaseType.Database, cwd: string | null, sessionId: string = 's1'): void {
  db.prepare(
    `INSERT OR IGNORE INTO sessions (session_id, project, jsonl_path, status, cwd)
     VALUES (?, 'test', '/fake/path.jsonl', 'dead', ?)`
  ).run(sessionId, cwd);
}
...
describe('discoverProjectDocs', () => {
  ...
  it('nonexistent cwd: handled silently, returns empty result, no throw', () => {
    ...
    let results: ReturnType<typeof discoverProjectDocs> = [];
