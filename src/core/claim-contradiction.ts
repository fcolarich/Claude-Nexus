 ...
 * handles separately.
 */
export function detectNumericContradiction(factA: string, factB: string): NumericContradiction | null {
	...
	if (normalizeSubject(subjectA) !== normalizeSubject(subjectB)) return null;
 ...
 */
export function writeContradictionLinks(db: Database.Database, claimIdA: string, claimIdB: string): void {
	const link = db.prepare(
		`INSERT OR IGNORE INTO memory_links (source_id, target_id, link_type, confidence) VALUES (?, ?, 'contradicts', 1.0)`
	);
