...
function unitAt(theta: number): Float32Array {
...
const noopEmbed = async (_text: string) => null; // never used when every claim is pre-embedded
...
describe('consolidateClaims', () => {
	...
	it('low-similarity claims (new band) get no edges at all', async () => {
		...
		const links = db.prepare(`SELECT COUNT(*) c FROM memory_links`).get() as { c: number };
