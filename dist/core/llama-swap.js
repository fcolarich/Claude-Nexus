 ...
 */
export async function ensureLlamaSwapReady(model, warmup, opts) {
...
async function _doEnsure(model, warmup, budgetMs) {
    ...
    if (!proxyUp) {
        ...
        if (executablePath && configPath) {
            try {
                const proc = spawn(executablePath, ['-config', configPath, '-listen', listenAddr()], { detached: true, stdio: 'ignore', shell: false });
...
//# sourceMappingURL=llama-swap.js.map