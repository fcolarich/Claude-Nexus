...
import type { Atom, TaskStatus } from './core/types.js';
...
function resolveEffectiveStatus(
  task: Pick<Atom, 'id' | 'status' | 'blocked_by'>,
  allTasksById: Map<string, Pick<Atom, 'id' | 'status'>>
): TaskStatus {
...
function makeTask(
  ...
  status: TaskStatus,
  blockedBy: string[] = []
): Pick<Atom, 'id' | 'status' | 'blocked_by'> {
...
describe('nexus_tasks filter: status=ready', () => {
  function getReadyTasks(
    tasks: Pick<Atom, 'id' | 'status' | 'blocked_by'>[]
  ): string[] {
