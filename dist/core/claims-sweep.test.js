...
import { decomposeMemoriesToClaims, countEligibleMemories } from './claims-sweep.js';
function freshDb() {
...
describe('decomposeMemoriesToClaims', () => {
    it('stamps claims_extracted_at on every processed memory, accepted or rejected, so a sweep terminates', async () => {
        ...
        const mixedFn = async (_s, user) => user.includes('body a') ? 'not json' : fakeCallFn();
        insertMemory(db, { ...baseMem, title: 'B', body: 'body b' });
    ...
    it('respects the limit — pool never exceeds it regardless of total eligible rows', async () => {
        const db = freshDb();
        for (let i = 0; i < 5; i++)
            insertMemory(db, { ...baseMem, title: `M${i}`, body: `body ${i}` });
...
//# sourceMappingURL=claims-sweep.test.js.map