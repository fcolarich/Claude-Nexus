...
export async function consolidateClaims(db, opts, embedFn = generateEmbedding) {
    const result = { embedded: 0, processed: 0, autoMerged: 0, flagged: 0, contradictions: 0 };
    ...
    const claims = db.prepare(sql).all(opts?.project ? { project: opts.project } : {});
    result.processed = claims.length;
    // Embed any claim lacking a vector before scoring.
    for (const c of claims) {
        const rowid = db.prepare(`SELECT rowid FROM claims WHERE id = ?`).get(c.id).rowid;
        const hasVec = db.prepare(`SELECT 1 FROM claims_vec WHERE rowid = ?`).get(rowid);
        if (!hasVec) {
            if (await embedClaim(db, c.id, embedFn))
                result.embedded++;
        }
    }
    const invalidatedThisRun = new Set();
    for (const claim of claims) {
        if (invalidatedThisRun.has(claim.id))
            continue;
        const queryClaim = { id: claim.id, memory_id: claim.memory_id, claim_type: claim.claim_type, fact: claim.fact };
        const candidates = findDedupCandidates(db, queryClaim);
        for (const candidate of candidates) {
            if (invalidatedThisRun.has(candidate.id) || invalidatedThisRun.has(claim.id))
                continue;
            ...
            if (band === 'auto_merge') {
                const candidateConfidence = db.prepare(`SELECT confidence FROM claims WHERE id = ?`).get(candidate.id).confidence;
...
//# sourceMappingURL=consolidate-claims.js.map