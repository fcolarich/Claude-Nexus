...
import { detectContradictions, MAX_PAIRS_PER_RUN } from './governance.js';
// ── Test setup ────────────────────────────────────────────────────────
...
function seedLink(db, sourceId, targetId, linkType, createdAt) {
    db.prepare(`
...
function countLinks(db, linkType) {
    const row = db.prepare(`SELECT COUNT(*) AS c FROM memory_links WHERE link_type = ?`).get(linkType);
    return row.c;
}
function countDiagnostics(db) {
    const row = db.prepare(`SELECT COUNT(*) AS c FROM diagnostics`).get();
    return row.c;
}
...
function makeConflictFake(reason = 'some reason') {
    return async () => JSON.stringify({ conflict: true, reason });
}
function getContradictsLink(db, sourceId, targetId) {
    return db.prepare(`
		SELECT source_id, target_id FROM memory_links
		WHERE link_type = 'contradicts' AND source_id = ? AND target_id = ?
	`).get(sourceId, targetId);
}
function getStaleDiagnostics(db) {
    return db.prepare(`SELECT type, atom_id, message, details FROM diagnostics WHERE type = 'stale'`).all();
}
...
describe('detectContradictions — confirmed writes', () => {
    it('writes bidirectional contradicts links and one stale diagnostic when haikuFn confirms a conflict', async () => {
        ...
        const details = JSON.parse(diag.details);
...
describe('detectContradictions — self-heal diagnostic re-derivation', () => {
    ...
    function wipeStaleDiagnostics(db) {
    ...
    it('leaves an already-contradicts pair alone when its diagnostic was never wiped (no duplicate)', async () => {
        ...
        db.prepare(`
			...
			VALUES ('stale', NULL, ?, ?, ?)
		`).run(`Contradiction candidate: "title-heal3-a" vs "title-heal3-b"`, JSON.stringify({ reason: 'contradiction', memory_ids: ['heal3-a', 'heal3-b'], haiku_reason: 'pre-existing' }), OLD_TIMESTAMP);
        expect(countDiagnostics(db)).toBe(1);
        const throwingFake = makeThrowingFake();
        await detectContradictions(db, throwingFake);
        expect(getStaleDiagnostics(db).length).toBe(1);
...
//# sourceMappingURL=governance-contradictions.test.js.map