 ...
 */
export async function ensureRerankerRunning(): Promise<void> {
	const cfg = getNexusConfig().reranker;
	if (!cfg.enabled || spawnAttempted) return;
	...
	for (let i = 0; i < 30; i++) {
		...
		try {
			const r = await fetch(`${base}/health`, { signal: AbortSignal.timeout(1000) });
			if (r.ok) { console.error('[reranker] local-reranker started'); return; }
