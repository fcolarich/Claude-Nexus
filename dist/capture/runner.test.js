...
function seedMemory(db, id, title) {
    db.prepare(`
...
describe('runFeedbackPass', () => {
    ...
    it('records feedback for unevaluated ids and marks the state file evaluated', async () => {
        ...
        const row1 = db.prepare(`SELECT use_count, help_count FROM memories WHERE id = 'mem-1'`).get();
        const row2 = db.prepare(`SELECT use_count, help_count FROM memories WHERE id = 'mem-2'`).get();
        ...
        expect(state.every((e) => e.evaluated === true)).toBe(true);
        db.close();
    });
    ...
    it('leaves ids unevaluated when the judge returns no verdicts (retried next firing)', async () => {
        ...
        const row = db.prepare(`SELECT use_count FROM memories WHERE id = 'mem-1'`).get();
